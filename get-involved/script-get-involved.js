...
if (mentorForm) {
  ...
  const roleInputs = Array.from(mentorForm.querySelectorAll('input[name="involvement_role"]'));
  const roleSections = {
    mentor: Array.from(mentorForm.querySelectorAll('[data-role-section="mentor"]')),
    volunteer: Array.from(mentorForm.querySelectorAll('[data-role-section="volunteer"]')),
  };

  const updateRoleSections = () => {
    const selectedRoles = roleInputs.filter((input) => input.checked).map((input) => input.value);
    const wantsMentor = selectedRoles.includes('Mentor') || selectedRoles.includes('Both');
    const wantsVolunteer = selectedRoles.includes('Volunteer') || selectedRoles.includes('Both');
    roleSections.mentor.forEach((section) => { section.hidden = !wantsMentor; });
    roleSections.volunteer.forEach((section) => { section.hidden = !wantsVolunteer; });
  };
  ...
  mentorForm.addEventListener('input', (event) => {
    ...
    if (target && target.name === 'involvement_role') {
      updateRoleSections();
    }
    saveDraft();
  });
